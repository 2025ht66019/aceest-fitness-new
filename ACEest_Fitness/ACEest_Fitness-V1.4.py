...
class FitnessTrackerApp:
    def __init__(self, master):
        ...
        # --- User & workout state ---
        self.user_info = {}  # name, regn-id, height, weight, age, gender, BMI, BMR
        ...
        self.style.configure("TNotebook.Tab", font=("Helvetica", 12, "bold"),
                             foreground=COLOR_TEXT, padding=[15, 8], background=COLOR_BACKGROUND)
        self.style.map("TNotebook.Tab",
                       background=[("selected", COLOR_CARD_BG)],
                       foreground=[("selected", COLOR_PRIMARY)])
        self.style.configure("Primary.TButton", font=("Arial", 11, "bold"),
                             background=COLOR_PRIMARY, foreground=COLOR_CARD_BG, padding=10)
        self.style.map("Primary.TButton", background=[('active', '#388E3C')])
        self.style.configure("Secondary.TButton", font=("Arial", 11, "bold"),
                             background=COLOR_SECONDARY, foreground=COLOR_CARD_BG, padding=10)
        self.style.map("Secondary.TButton", background=[('active', '#1976D2')])

        # --- Root grid layout ---
        self.master.grid_rowconfigure(0, weight=1)
        self.master.grid_columnconfigure(0, weight=0)  # sidebar (fixed width)
        self.master.grid_columnconfigure(1, weight=1)  # notebook (expands)

        # Left sidebar (User Info)
        self.sidebar = tk.Frame(self.master, bg=COLOR_CARD_BG, padx=20, pady=15, relief=tk.RIDGE, bd=2)
        self.sidebar.grid(row=0, column=0, sticky="n", padx=(20, 10), pady=20)

        # Notebook on the right
        self.notebook = ttk.Notebook(self.master)
        self.notebook.grid(row=0, column=1, sticky="nsew", padx=(10, 20), pady=20)
        ...
        # Build sections
        self.create_user_info_section(parent=self.sidebar)
        self.create_log_tab()
        self.create_workout_plan_tab()
        self.create_diet_guide_tab()
        self.create_progress_tab()

        # Keyboard shortcuts to submit user info quickly
        self.master.bind("<Control-s>", lambda e: self.save_user_info())
    ...
    def create_user_info_section(self, parent):
        title = tk.Label(parent, text="📝 User Info", font=("Inter", 14, "bold"), bg=COLOR_CARD_BG)
        title.grid(row=0, column=0, columnspan=2, sticky="w", pady=(0, 8))

        parent.grid_columnconfigure(0, weight=0)
        parent.grid_columnconfigure(1, weight=1)  # entries expand

        # Name
        tk.Label(parent, text="Name:", bg=COLOR_CARD_BG).grid(row=1, column=0, sticky="w", pady=3)
        self.name_entry = tk.Entry(parent)
        self.name_entry.grid(row=1, column=1, sticky="ew", pady=3)
        self.name_entry.bind("<Return>", lambda e: self.save_user_info())

        # Regn-ID
        tk.Label(parent, text="Regn-ID:", bg=COLOR_CARD_BG).grid(row=2, column=0, sticky="w", pady=3)
        self.regn_entry = tk.Entry(parent)
        self.regn_entry.grid(row=2, column=1, sticky="ew", pady=3)
        self.regn_entry.bind("<Return>", lambda e: self.save_user_info())

        # Age
        tk.Label(parent, text="Age:", bg=COLOR_CARD_BG).grid(row=3, column=0, sticky="w", pady=3)
        self.age_entry = tk.Entry(parent)
        self.age_entry.grid(row=3, column=1, sticky="ew", pady=3)
        self.age_entry.bind("<Return>", lambda e: self.save_user_info())

        # Gender
        tk.Label(parent, text="Gender (M/F):", bg=COLOR_CARD_BG).grid(row=4, column=0, sticky="w", pady=3)
        self.gender_entry = tk.Entry(parent)
        self.gender_entry.grid(row=4, column=1, sticky="ew", pady=3)
        self.gender_entry.bind("<Return>", lambda e: self.save_user_info())

        # Height
        tk.Label(parent, text="Height (cm):", bg=COLOR_CARD_BG).grid(row=5, column=0, sticky="w", pady=3)
        self.height_entry = tk.Entry(parent)
        self.height_entry.grid(row=5, column=1, sticky="ew", pady=3)
        self.height_entry.bind("<Return>", lambda e: self.save_user_info())

        # Weight
        tk.Label(parent, text="Weight (kg):", bg=COLOR_CARD_BG).grid(row=6, column=0, sticky="w", pady=3)
        self.weight_entry = tk.Entry(parent)
        self.weight_entry.grid(row=6, column=1, sticky="ew", pady=3)
        self.weight_entry.bind("<Return>", lambda e: self.save_user_info())

        # Buttons
        btn_save = ttk.Button(parent, text="Save Info", command=self.save_user_info, style="Primary.TButton")
        btn_save.grid(row=7, column=0, columnspan=2, sticky="ew", pady=(10, 6))

        btn_export = ttk.Button(parent, text="📄 Export Weekly PDF Report",
                                command=self.export_weekly_report, style="Secondary.TButton")
        btn_export.grid(row=8, column=0, columnspan=2, sticky="ew")
    ...
    def create_log_tab(self):
        tk.Label(self.log_tab, text="ACEest Session Logger", font=("Inter", 20, "bold"),
                 bg=COLOR_BACKGROUND, fg=COLOR_TEXT).pack(pady=(20, 10))
        tk.Label(self.log_tab, text="Track your progress with precision.", font=("Inter", 12),
                 bg=COLOR_BACKGROUND, fg="#6C757D").pack(pady=(0, 20))
        ...
        tk.Label(log_card, text="Category:", font=("Inter", 12, "bold"), bg=COLOR_CARD_BG, fg=COLOR_TEXT)\
        ...
        self.category_menu = ttk.Combobox(log_card, textvariable=self.category_var,
                                          ...
                                          width=30, font=("Inter", 11))
        self.category_menu.grid(row=0, column=1, sticky="w", padx=10, pady=10)

        # Exercise
        tk.Label(log_card, text="Exercise Name:", font=("Inter", 12, "bold"), bg=COLOR_CARD_BG, fg=COLOR_TEXT)\
        ...
        # Duration
        tk.Label(log_card, text="Duration (min):", font=("Inter", 12, "bold"), bg=COLOR_CARD_BG, fg=COLOR_TEXT)\
        ...
        ttk.Button(button_frame, text="✅ ADD SESSION", command=self.add_workout,
                    style="Primary.TButton", width=18).grid(row=0, column=0, padx=15)
        ttk.Button(button_frame, text="🗂️ VIEW SUMMARY", command=self.view_summary,
                    style="Secondary.TButton", width=18).grid(row=0, column=1, padx=15)

        self.status_label = tk.Label(self.log_tab, text="Welcome! Ready for a great session.", bd=1,
                                     relief=tk.FLAT, anchor=tk.W, bg=COLOR_CARD_BG, fg="#6C757D",
                                     font=("Inter", 10))
    ...
    def view_summary(self):
        ...
        tk.Label(summary_window, text="🏋️ Full Session History", font=("Inter", 16, "bold"),
                 bg=COLOR_CARD_BG, fg=COLOR_TEXT).pack(pady=10)
        text_frame = tk.Frame(summary_window, bg=COLOR_CARD_BG)
        text_frame.pack(pady=10, padx=20, fill="both", expand=True)
        scrollbar = ttk.Scrollbar(text_frame)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        summary_text = tk.Text(text_frame, height=20, width=55, wrap=tk.WORD, font=("Inter", 10),
                               bg=COLOR_BACKGROUND, fg=COLOR_TEXT, yscrollcommand=scrollbar.set,
                               relief=tk.FLAT)
        ...
        for category, sessions in self.workouts.items():
            ...
            summary_text.tag_config(category.lower(), font=("Inter", 12, "bold"),
                                    foreground=COLOR_SECONDARY if category=="Warm-up" else
                                               COLOR_PRIMARY if category=="Workout" else "#FFC107")
            if sessions:
                for i, entry in enumerate(sessions, 1):
                    line = (f" {i}. {entry['exercise']} - {entry['duration']} min\n"
                            f"    {entry['calories']:.1f} kcal\n"
                            f"    Date: {entry['timestamp'].split(' ')[0]}\n")
        ...
        summary_text.insert(tk.END, "--- LIFETIME TOTALS ---\n", "total_header")
    ...
    def create_progress_tab(self):
        tk.Label(self.progress_tab, text="📈 Personal Progress Tracker", font=("Inter", 20, "bold"),
                 bg=COLOR_CARD_BG, fg=COLOR_TEXT).pack(pady=(20, 10))
        tk.Label(self.progress_tab, text="Visualization of your logged workout time distribution.",
                 font=("Inter", 12), bg=COLOR_CARD_BG, fg="#6C757D").pack(pady=(0, 20))
    ...
    def update_progress_charts(self):
        ...
        if sum(values) == 0:
            tk.Label(self.chart_container, text="No workout data logged yet.",
        ...
        ax2.pie(pie_values, labels=pie_labels, autopct="%1.1f%%", startangle=90, colors=pie_colors,
                ...
                textprops={'fontsize': 8, 'color': COLOR_TEXT})
        ...
        tk.Label(self.progress_tab, text=f"LIFETIME TOTAL: {total_minutes} minutes logged",
                 font=("Inter", 13, "bold"), bg=COLOR_CARD_BG, fg="#DC3545").pack(pady=(10, 5))
    ...
    def export_weekly_report(self):
        if not self.user_info:
            messagebox.showerror("Error", "Please save user info first!")
            return
        filename = f"{self.user_info['name'].replace(' ', '_')}_weekly_report.pdf"
        c = pdf_canvas.Canvas(filename, pagesize=A4)
        width, height = A4
        c.setFont("Helvetica-Bold", 16)
        c.drawString(50, height-50, f"Weekly Fitness Report - {self.user_info['name']}")

        # User Info
        c.setFont("Helvetica", 11)
        c.drawString(50, height-80,
                     f"Regn-ID: {self.user_info['regn_id']}    Age: {self.user_info['age']}    Gender: {self.user_info['gender']}")
        c.drawString(50, height-100,
                     f"Height: {self.user_info['height']} cm    Weight: {self.user_info['weight']} kg    "
                     f"BMI: {self.user_info['bmi']:.1f}    BMR: {self.user_info['bmr']:.0f} kcal/day")
